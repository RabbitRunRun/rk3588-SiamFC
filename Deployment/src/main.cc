...
/*
static int image_resize(uint8_t *src_img, int srcw, int srch, uint8_t *dst_img, int dstw, int dsth)
{
       ...
       //uint8_t *src_buf = (uint8_t *)malloc(srcw * srch * 3);



       rga_buffer_handle_t src_handle,dst_handle;
       src_handle = importbuffer_virtualaddr(src_img, srcw * srch * 3);
       dst_handle = importbuffer_virtualaddr(dst_img, dstw * dsth * 3);

       if(src_handle == 0 || dst_handle == 0)
       {
            std::cout << "image_resize failed, importbuffer_virtualaddr failed" << std::endl;
            return -1;
       } 

       src = wrapbuffer_handle(src_handle, srcw, srch, RK_FORMAT_RGB_888);
       dst = wrapbuffer_handle(dst_handle, dstw, dsth, RK_FORMAT_RGB_888);
       //src = wrapbuffer_virtualaddr((void*)src_img, srcw, srch, RK_FORMAT_RGB_888);
       //dst = wrapbuffer_virtualaddr((void*)dst_img, dstw, dsth, RK_FORMAT_RGB_888);
       auto ret = imcheck(src, dst, {}, {});

       releasebuffer_handle(src_handle);
       releasebuffer_handle(dst_handle);

       if (IM_STATUS_NOERROR != ret) {
          printf("%d,image_resize check error! %s", __LINE__, imStrError((IM_STATUS)ret));
          return -1;
       }
...
static std::vector<uint8_t> preprocess(const SeetaAIPImageData &image) {
    ...
    */
    ...
    vec.resize(image.height * image.width * image.channels);
...
static std::vector<uint8_t> get_subwindow(SeetaAIPImageData &img, std::array<int,2> &pos, int model_sz, int original_sz, uint8_t (&c)[3])
{
     std::chrono::system_clock::time_point begintime = std::chrono::system_clock::now();
     ...
     if (model_sz != original_sz)
     {
          ...
          //{
          //    std::cout << "-----get_sub_window-----image_resize failed" << std::endl;
          //    return ten;
          //}
...
Tracker::Tracker(const std::string &fmodel, const std::string &mmodel)
{
     m_anchor_num = 5;
...
Tracker::~Tracker()
{
    //rknn_outputs_release(m_fctx, m_f_io_num.n_output, m_f_outputs);
    rknn_destroy(m_fctx);
    rknn_destroy(m_mctx);
...
int Tracker::init()
{
    ...
    std::cout << std::endl;
    std::cout << "template output attr: " << std::endl;
    ...
    /*
    m_f_outputs = new rknn_output[m_f_io_num.n_output];
    memset(m_f_outputs, 0, sizeof(rknn_output) * m_f_io_num.n_output);
    for (int i = 0; i < m_f_io_num.n_output; i++) 
    {
        m_f_outputs[i].want_float = 1;
    }
    */
...
int Tracker::set_template(const cv::Mat &mat,cv::Rect bbox)
{
    std::chrono::system_clock::time_point begintime = std::chrono::system_clock::now();
    ...
    std::vector<uint8_t> vec = get_subwindow(s, m_center_pos, m_template_size[0], s_z, m_channel_average);
    ...
    */
    ...
    //m_f_inputs[0].buf = (void*)mat.data;//(void *)z_crop.data();
    //m_f_inputs[0].size = mat.cols * mat.rows * mat.channels();
...
ScoreRect Tracker::track(const cv::Mat &mat)
{
    ...
    //m_m_inputs[1].buf = input_buf2.data(); //m_f_outputs[1].buf; 
    //m_m_inputs[1].size = m_f_outputs[1].size;

    //m_m_inputs[2].buf = input_buf3.data(); //m_f_outputs[2].buf; 
    //m_m_inputs[2].size = m_f_outputs[2].size;

    m_m_inputs[3].buf = (void *)vec.data(); //x_crop.data(); 
    m_m_inputs[3].size = vec.size();//x_crop.count();//x_crop.count();
   
    //m_m_inputs[3].buf = (void *)mat.data; //x_crop.data(); 
    //m_m_inputs[3].size =  mat.cols * mat.rows * mat.channels(); 
    ...
    */
    ...
    printf("track ok, spend time:%ld ms\n", duration.count());
    return r;
}


ScoreRect Tracker::postprocess(
...
void usage()
{
    std::cout << " use: template_model, search_model, timage, simage" << std::endl;
}
